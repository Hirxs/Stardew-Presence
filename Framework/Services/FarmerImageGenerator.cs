    ...
    public class FarmerImageGenerator : IDisposable
    {
        ...
        public void CheckAndUpdate(Farmer farmer, Action<string> onUrlUpdated)
        {
            ...
            try
            {
                ...
                {
                    monitor.Log("[StardewDiscordRPC] Failed to render farmer portrait canvas.", LogLevel.Trace);
                    return;
                }
                ...
                try
                {
                    ...
                    ModLogger.LogInfo(monitor, $"[StardewDiscordRPC] Saved local image preview to: {localFilePath}");
                }
                catch (Exception ex)
                {
                    ModLogger.LogTrace(monitor, $"[StardewDiscordRPC] Could not save local preview: {ex.Message}");
                }
                ...
                Task.Run(async () =>
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(url))
                        {
                            ...
                            ModLogger.LogInfo(monitor, $"[StardewDiscordRPC] Uploaded dynamic farmer card to: {url}");
                            onUrlUpdated?.Invoke(url);
                        }
                    }
                    catch (Exception ex)
                    {
                        monitor.Log($"[StardewDiscordRPC] Error uploading dynamic image: {ex.Message}", LogLevel.Warn);
                    }
                    finally
                    {
                        isUploading = false;
            ...
            {
                ModLogger.LogTrace(monitor, $"[StardewDiscordRPC] Render error: {ex.Message}");
            }
        }
