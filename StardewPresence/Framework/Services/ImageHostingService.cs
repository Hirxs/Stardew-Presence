...
namespace StardewPresence.Framework.Services
{
    ...
    public class ImageHostingService : IDisposable
    {
        ...
        public async Task<string?> UploadAsync(byte[] pngBytes, ModConfig config)
        {
            ...
            ModLogger.LogTrace(monitor, "[StardewPresence] Using fallback Catbox image upload...");
        ...
        private async Task<string?> UploadCatboxAsync(byte[] pngBytes)
        {
            ...
            {
                ModLogger.LogTrace(monitor, $"[StardewPresence] Catbox upload error: {ex.Message}");
            }
            return null;
        }
        ...
        private async Task<string?> UploadImgBbAsync(byte[] pngBytes, string apiKey)
        {
            ...
            {
                ModLogger.LogTrace(monitor, $"[StardewPresence] ImgBB upload error: {ex.Message}");
            }
            return null;
        }
        ...
        private async Task<string?> UploadFreeimageAsync(byte[] pngBytes, string? customApiKey)
        {
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    ModLogger.LogTrace(monitor, $"[StardewPresence] Freeimage.host HTTP {(int)response.StatusCode}: {errBody}");
                    return null;
                }
            ...
            {
                ModLogger.LogTrace(monitor, $"[StardewPresence] Freeimage upload exception: {ex.Message}");
            }
