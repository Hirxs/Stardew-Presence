using System;
using System.Linq;
using StardewDiscordRPC.Framework.Clients;
using StardewDiscordRPC.Framework.Integrations;
using StardewDiscordRPC.Framework.Menus;
using StardewDiscordRPC.Framework.Models;
using StardewDiscordRPC.Framework.Presence;
...
namespace StardewDiscordRPC
{
    public class ModEntry : Mod
    {
        ...
        private void OnGameLaunched(object? sender, GameLaunchedEventArgs e)
        {
            ...
            ModLogger.LogInfo(Monitor, $"[StardewDiscordRPC] Detected {totalMods} active mods.");
        ...
        private void InitDiscordClient()
        {
            ...
            {
                Monitor.Log($"[StardewDiscordRPC] Failed to initialize Discord RPC: {ex}", LogLevel.Error);
            }
        }

        private void OnDiscordJoin(string secret)
        {
            ModLogger.LogInfo(Monitor, $"[StardewDiscordRPC] Discord Join invitation received with secret/code: {secret}");
            ...
            {
                ModLogger.LogTrace(Monitor, $"[StardewDiscordRPC] Could not process Discord join secret: {ex.Message}");
            }
        }

        private void OnDiscordJoinRequest(DiscordUser user)
        {
            ModLogger.LogInfo(Monitor, $"[StardewDiscordRPC] Player '{user.Username}' requested to join your farm via Discord.");
            try
            {
        ...
        private void UpdatePresence()
        {
            ...
            {
                ModLogger.LogTrace(Monitor, $"[StardewDiscordRPC] Error building presence update: {ex}");
            }
        }
        ...
        private void OpenEditorMenu()
        {
            ...
            {
                Monitor.Log("[StardewDiscordRPC] You must load a save file before opening the Portrait Editor.", LogLevel.Warn);
                return;
            }
        ...
        private void OpenMapBackgroundSelector()
        {
            if (!Context.IsWorldReady || farmerImageGenerator == null)
            {
                Monitor.Log("[StardewDiscordRPC] You must load a save file before opening the Map Background Selector.", LogLevel.Warn);
                return;
            }
        ...
        private void RefreshCustomBackground()
        {
            ...
            {
                Monitor.Log("[StardewDiscordRPC] You must load a save file before refreshing the map background.", LogLevel.Warn);
                return;
            }
            ...
            if (farmerImageGenerator != null)
            {
                ...
                if (ok)
                {
                    ModLogger.LogInfo(Monitor, $"[StardewDiscordRPC] Successfully refreshed map background spot at '{config.CustomMapLocation}' ({config.CustomMapTileX}, {config.CustomMapTileY}) for season '{Game1.currentSeason}'.");
                    UpdateFarmerImage();
                    UpdatePresence();
                }
                else
                {
                    Monitor.Log("[StardewDiscordRPC] Could not refresh map background. Ensure you have captured a spot first.", LogLevel.Warn);
                }
            }
        }
